...
-- ============================================================
insert into public.products (emoji,name,cat,price,cost,sales,trend,creators,shops,reviews,rating,days,live_share,f_wow,f_demo,f_problem,f_easy)
select * from (values
  ('👁️','เครื่องนวดตาไฟฟ้าพกพา','สุขภาพ',390,150,420,145,12,34,210,4.7,22,0,true,true,true,true),
  ('👃','ที่ดูดน้ำมูกเด็กไฟฟ้า','แม่และเด็ก',290,95,260,95,20,45,130,4.8,18,5,false,true,true,true),
  ('🌇','โคมไฟพระอาทิตย์ตั้งโต๊ะ','ของใช้ในบ้าน',220,70,800,20,480,520,2100,4.6,120,45,true,true,false,true),
  ('🪥','แปรงสีฟันไฟฟ้าเด็กลายการ์ตูน','สุขภาพ',180,60,150,60,40,90,85,4.7,30,20,false,true,true,true),
  ('🧼','เครื่องตีโฟมล้างหน้าอัตโนมัติ','ความงาม',260,80,340,110,28,60,160,4.6,20,15,true,true,false,true),
  ('🔌','คลิปหนีบสายไฟแม่เหล็กติดโต๊ะ','แกดเจ็ต',120,35,90,25,60,140,70,4.5,40,0,false,true,false,true),
  ('💆','หวีนวดหนังศีรษะไฟฟ้า','ความงาม',350,120,300,130,18,40,140,4.7,16,10,true,true,true,true),
  ('🍶','กระบอกน้ำมีจอวัดอุณหภูมิ LED','แกดเจ็ต',320,110,500,40,220,300,900,4.6,80,40,true,false,false,true),
  ('🦟','เครื่องดักยุงไฟฟ้า USB เงียบ','ของใช้ในบ้าน',450,160,380,150,25,55,175,4.5,20,0,true,true,true,true),
  ('👀','แผ่นแปะลดบวมใต้ตา (30 คู่)','ความงาม',150,45,260,55,90,180,400,4.4,50,30,false,true,true,true),
  ('🥤','เครื่องปั่นพกพาไร้สาย','ครัว',480,180,210,35,350,400,1500,4.5,150,55,true,true,false,true),
  ('🐾','ที่กรอเล็บสุนัขไฟฟ้าเงียบ','สัตว์เลี้ยง',340,110,180,120,15,30,95,4.7,17,0,false,true,true,true)
) as v(emoji,name,cat,price,cost,sales,trend,creators,shops,reviews,rating,days,live_share,f_wow,f_demo,f_problem,f_easy)
